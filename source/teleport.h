...
class Teleport : public Item, public Cylinder
{
	public:
		...
		virtual ReturnValue __queryAdd(int32_t, const Thing*, uint32_t, uint32_t, Creature* = NULL) const {
			return RET_NOTPOSSIBLE;
		}
		virtual ReturnValue __queryMaxCount(int32_t index, const Thing* thing, uint32_t count,
			uint32_t& maxQueryCount, uint32_t flags) const {return RET_NOTPOSSIBLE;}
		virtual ReturnValue __queryRemove(const Thing*, uint32_t, uint32_t, Creature* = NULL) const {return RET_NOERROR;}
